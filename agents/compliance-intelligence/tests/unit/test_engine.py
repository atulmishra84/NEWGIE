...
from compliance_intelligence.domain.engine import analyze_compliance, determine_applicability

def test_catalog_has_all_frameworks():
    cat = load_catalog()
    expected = {
        "hipaa", "gdpr", "pci_dss", "soc2", "iso27001", "nist_ai_rmf", "nist_csf",
        "eu_ai_act", "fda", "rbi", "mas", "dora", "ccpa", "internal_corporate",
    }
...
def test_applicability_and_gaps(sample_bundle):
    ...
    assert any(a.control_id == "hipaa-access" and a.status == ControlStatus.PARTIAL for a in report.assessments)
