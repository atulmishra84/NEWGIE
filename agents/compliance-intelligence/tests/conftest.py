...
os.environ.setdefault(
    ...
    str(Path(__file__).resolve().parents[1] / "data" / "frameworks" / "catalog_v1.json"),
)
...
def sample_bundle():
    return ComplianceInputBundle(
        ...
        context_model={
            "ai": {"frameworks": [{"name": "langgraph"}, {"name": "openai"}], "models": [{"name": "gpt-4o"}]},
        ...
        risk_report={"overall_ai_risk_score": 0.62, "factors": [{"category": "privacy", "score": 0.7}]},
        knowledge={"hits": [{"node_id": "hipaa"}, {"node_id": "gdpr"}]},
        identity={"providers": ["okta"], "mfa": True},
        business={"industry": "healthcare", "criticality": "critical", "regions": ["US", "EU"]},
        declared_frameworks=[FrameworkId.HIPAA, FrameworkId.GDPR, FrameworkId.SOC2],
        implemented_controls=["hipaa-access"],
        evidence=[],
        internal_policies=[{"id": "pol-shadow-ai", "title": "No unsanctioned AI tools"}],
    )
