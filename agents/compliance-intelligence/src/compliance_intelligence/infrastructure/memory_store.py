...
class InMemoryComplianceReportRepository(ComplianceReportRepository):
    ...
    async def save(self, report: ComplianceReport) -> None:
        ...
        self._by_app.setdefault((report.tenant_id, report.application_id), []).append(report.report_id)

    async def get(self, report_id: UUID) -> ComplianceReport | None:
        return self._by_id.get(report_id)

    async def latest(self, tenant_id: str, application_id: str) -> ComplianceReport | None:
        ids = self._by_app.get((tenant_id, application_id)) or []
        return self._by_id.get(ids[-1]) if ids else None

    async def list(self, tenant_id: str, limit: int = 50, offset: int = 0) -> list[ComplianceReport]:
        items = [self._by_id[i] for (t, _), ids in self._by_app.items() if t == tenant_id for i in ids]
