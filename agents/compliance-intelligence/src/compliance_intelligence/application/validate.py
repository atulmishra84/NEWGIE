...
class ValidateComplianceHandler:
    ...
    async def handle(
        self, request: ComplianceValidateRequest, *, actor: str, correlation_id: str
    ):
        prior = await self._reports.latest(request.tenant_id, request.application_id)
        if not prior:
            raise NotFoundError(
                f"No compliance report for application {request.application_id}; run analyze first"
            )
        ...
        missing = sum(
            1 for a in report.assessments if a.status == ControlStatus.MISSING
        )
        validated = sum(
            1 for a in report.assessments if a.status == ControlStatus.IMPLEMENTED
        )
