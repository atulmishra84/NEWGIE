...
async def _analyze(input_json: Path, out: Path):
    ...
    bundle = ComplianceInputBundle.model_validate(raw if "application_id" in raw else raw.get("bundle", raw))
    c = await build_container(memory=True)
    report = await c.analyze.handle(ComplianceAnalyzeRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex)
