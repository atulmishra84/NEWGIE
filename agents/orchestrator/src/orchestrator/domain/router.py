...
def resolve_version(
    agent_id: AgentId, agent_versions: dict[str, str], step_version: str | None = None
) -> str:
    if step_version:
        return step_version
    return (
        agent_versions.get(agent_id.value)
        or agent_versions.get(agent_id.value.replace("_", "-"))
        or "1.0.0"
    )
    ...
    for agent_id, url in urls.items():
        ...
        out.append(
            AgentEndpoint(
                ...
                base_url=url,
                version="1.0.0",
                healthy=core or settings.simulate_agents,
                latency_ms_p95=40.0 if core else 80.0,
            )
        )
    return out
