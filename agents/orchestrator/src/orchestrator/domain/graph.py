...
def default_analyze_workflow(*, parallel_enabled: bool = True) -> WorkflowDefinition:
    ...
    steps = [
        ...
        StepDefinition(
            ...
            depends_on=["risk", "compliance"],
            timeout_ms=30_000,
        ),
        StepDefinition(
            step_id="recommendation",
            agent_id=AgentId.RECOMMENDATION,
            depends_on=["policy"],
            timeout_ms=30_000,
        ),
        StepDefinition(
        ...
        StepDefinition(
            ...
            agent_id=AgentId.EXPLAINABILITY,
...
def sequential_analyze_workflow() -> WorkflowDefinition:
    ...
    for agent in order:
        ...
        prev = sid
    return WorkflowDefinition(
        workflow_id="gie.analyze.sequential",
        name="GIE Sequential Analysis",
        steps=steps,
...
def topological_waves(workflow: WorkflowDefinition) -> list[list[StepDefinition]]:
    ...
    while remaining:
        ready = [
            s for s in remaining.values() if all(d in completed for d in s.depends_on)
        ]
        if not ready:
