...
from orchestrator.application.di import Container, get_container
...
def require_perm(permission: OrchestratorPermission):
    async def _inner(principal: AuthPrincipal = Depends(get_principal)) -> AuthPrincipal:
        require_orch_permission(principal.roles, permission)
        return principal
    return _inner
