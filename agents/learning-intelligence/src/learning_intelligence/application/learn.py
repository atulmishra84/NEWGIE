...
class LearnHandler:
    ...
    async def handle(
        self, request: LearnRequest, *, actor: str, correlation_id: str
    ) -> LearningReport:
        ...
        report = run_learning_cycle(
            request.bundle, allow_auto_publish=self._settings.allow_auto_publish
        )
        if request.persist:
        ...
        logger.info(
            "learning_cycle_completed", learning_id=str(report.learning_id), actor=actor
        )
        return report
