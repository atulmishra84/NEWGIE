...
async def _learn(input_json: Path, out: Path):
    ...
    bundle = LearningInputBundle.model_validate(raw if "tenant_id" in raw else raw.get("bundle", raw))
    c = await build_container(memory=True)
    report = await c.learn.handle(LearnRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex)
