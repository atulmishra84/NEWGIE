...
TOOLS = [
    ...
    },
    {
        ...
        "description": "List knowledge changes",
        "inputSchema": {
            "type": "object",
            "properties": {"tenant_id": {"type": "string"}},
            "required": ["tenant_id"],
...
async def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "learning_feedback":
        ...
        return await c.feedback_handler.handle(
            FeedbackRequest(event=event), actor="mcp", correlation_id=uuid4().hex
        )
    if name == "learning_learn":
        bundle = LearningInputBundle.model_validate(arguments["bundle"])
        report = await c.learn.handle(
            LearnRequest(bundle=bundle), actor="mcp", correlation_id=uuid4().hex
        )
    ...
    if name == "knowledge_changes":
        items = await c.knowledge.list(
            arguments["tenant_id"], status=arguments.get("status")
        )
        return {"items": [i.model_dump(mode="json") for i in items]}
