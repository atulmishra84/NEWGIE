...
def test_learning_improves_and_proposes(sample_bundle):
    ...
    assert (
        "new_attack_technique" in kinds
        or "regulation_change" in kinds
        or "policy_drift" in kinds
    )
    assert report.policy_updates
    assert report.knowledge_changes
    assert all(
        k.status == KnowledgeChangeStatus.PROPOSED for k in report.knowledge_changes
    )
