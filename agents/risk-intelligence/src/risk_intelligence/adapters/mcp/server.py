...
TOOLS = [
    {
        "name": "risk_calculate",
        "description": "Calculate overall AI risk posture",
        "inputSchema": {
            "type": "object",
    ...
    {
        "name": "risk_recalculate",
        "description": "Dynamically recalculate risk for an agent",
        "inputSchema": {
            "type": "object",
            "properties": {
                "agent_id": {"type": "string"},
                "tenant_id": {"type": "string"},
                "org_risk_model": {"type": "object"},
            },
    ...
    {
        "name": "risk_get",
        "description": "Get latest risk report for an agent",
        "inputSchema": {
            "type": "object",
            "properties": {
                "agent_id": {"type": "string"},
...
async def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "risk_calculate":
        ...
        report = await c.calculate.handle(
            RiskCalculateRequest(bundle=bundle), actor="mcp", correlation_id=uuid4().hex
        )
    ...
    if name == "risk_recalculate":
        ...
        report = await c.recalculate.handle(
            req, actor="mcp", correlation_id=uuid4().hex
        )
        return report.model_dump(mode="json")
    if name == "risk_get":
        report = await c.reports.latest_for_agent(
    ...
    if name == "risk_remediation":
        ...
        return (
            {"remediations": [r.model_dump() for r in report.remediations]}
            if report
            else {}
        )
