...
async def _calc(input_json: Path, out: Path):
    ...
    bundle = RiskInputBundle.model_validate(
        raw if "agent_id" in raw else raw.get("bundle", raw)
    )
    c = await build_container(memory=True)
    report = await c.calculate.handle(
        RiskCalculateRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex
    )
    out.write_text(report.model_dump_json(indent=2))
    rprint(
        f"overall={report.overall_ai_risk_score} trust={report.trust_score} severity={report.severity.value}"
    )
    rprint(f"wrote {out}")
