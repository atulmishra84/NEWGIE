...
class Container:
    ...
    def calculate(self) -> CalculateRiskHandler:
        return CalculateRiskHandler(reports=self.reports, cache=self.cache, events=self.events, settings=self.settings)

    @property
    def recalculate(self) -> RecalculateRiskHandler:
        return RecalculateRiskHandler(reports=self.reports, calculate=self.calculate, cache=self.cache)
