...
def _factor(
    ...
    return RiskFactor(
        ...
        confidence=Confidence(
            score=min(0.95, 0.6 + 0.05 * len(evidence)),
            rationale="signal-weighted heuristic",
        ),
...
def score_prompt_injection(
    ...
    return _factor(
        ...
        [
            f"frameworks={sig['frameworks']}",
            f"prompt_risk={sig['prompt_risk']}",
            f"mcp={sig['has_mcp']}",
        ],
        [
            _rem(
                "rem-pi-filter",
                "Deploy prompt-injection input filters",
                "Add instruction hierarchy and untrusted content isolation.",
                ["gr-prompt-injection"],
                "P0",
            )
        ],
...
def score_jailbreak(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.2
        + (0.4 if sig["jailbreak_markers"] else 0.0)
        + (0.15 if sig["has_autonomy"] else 0.0)
    )
    ...
    return _factor(
        ...
        [
            _rem(
                "rem-jb",
                "Enable safety/topic blocklists and output filters",
                "Apply vendor safety + output scanning.",
                ["gr-topic-safety", "gr-output-filter"],
            )
        ],
        w,
    )
...
def score_tool_abuse(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.15
        + 0.15 * min(3, len(sig["dangerous_tools"]))
        + (0.2 if sig["has_mcp"] else 0)
        + (0.15 if sig["has_autonomy"] else 0)
    )
    ...
    return _factor(
        ...
        [
            _rem(
                "rem-tools",
                "Enforce tool allowlists and human approval",
                "Deny shell/exec; require approval for high-impact tools.",
                ["gr-tool-allowlist"],
                "P0",
            )
        ],
...
def score_data_leakage(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.2
        + (0.3 if sig["has_pii_context"] else 0)
        + (0.25 if sig["secrets_detected"] else 0)
        + (0.1 if sig["public_runtime"] else 0)
    )
    ...
    return _factor(
        ...
        [
            f"pii={sig['has_pii_context']}",
            f"secrets={sig['secrets_detected']}",
            f"public={sig['public_runtime']}",
        ],
        [
            _rem(
                "rem-dlp",
                "Enable DLP/Presidio and output filters",
                "Scan prompts/completions/logs for sensitive data.",
                ["gr-pii-presidio", "gr-output-filter"],
                "P0",
            )
        ],
        w,
...
def score_privacy(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.2
        + (0.35 if sig["has_pii_context"] else 0.1)
        + (0.1 if "gdpr" in sig["haystack"] or "hipaa" in sig["haystack"] else 0)
    )
    ...
    return _factor(
        ...
        "Privacy Risk",
        base * w,
        "Regulated personal data in AI workflows creates privacy exposure.",
        [f"pii_context={sig['has_pii_context']}"],
        [
            _rem(
                "rem-privacy",
                "Apply privacy-by-design controls",
                "Minimize data, anonymize, retain lawfully.",
                ["gr-pii-presidio"],
            )
        ],
        w,
...
def score_compliance(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    base = (
        0.15
        + min(0.5, 0.08 * len(fws if isinstance(fws, list) else [fws]))
        + (0.2 if sig["has_pii_context"] and not fws else 0.1)
    )
    ...
    return _factor(
        ...
        "Compliance Risk",
        base * w,
        "Obligations from compliance frameworks require mapped AI controls.",
        [f"frameworks={fws}"],
        [
            _rem(
                "rem-comp",
                "Map controls to obligations",
                "Align guardrails to SOC2/GDPR/EU AI Act/HIPAA as applicable.",
...
def score_identity(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    return _factor(
        ...
        "Identity Risk",
        base * w,
        "Weak or missing strong identity for AI agents increases privilege abuse risk.",
        [f"weak_identity={sig['weak_identity']}"],
        [
            _rem(
                "rem-id",
                "Enforce workload identity and agent RBAC",
                "Use OAuth/OIDC, short-lived tokens, least privilege.",
                ["gr-identity-rbac"],
                "P1",
            )
        ],
...
def score_security(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.25
        + (0.2 if sig["secrets_detected"] else 0)
        + (0.15 if sig["public_runtime"] else 0)
        + (0.1 if sig["dangerous_tools"] else 0)
    )
    ...
    return _factor(
        ...
        "Security Risk",
        base * w,
        "Composite security exposure from secrets, tools, and exposure posture.",
        ["composite security signals"],
        [
            _rem(
                "rem-sec",
                "Hardening baseline",
                "Secrets hygiene, network controls, output validation.",
                ["gr-output-filter", "gr-opa-runtime"],
            )
        ],
        w,
    )
...
def score_model(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.2
        + min(0.3, 0.08 * sig["model_count"])
        + (
            0.15
            if "fine-tune" in sig["haystack"] or "weights" in sig["haystack"]
            else 0
        )
    )
    ...
    return _factor(
        ...
        "Model Risk",
        base * w,
        "Model inventory size and customization increase model risk.",
        [f"model_count={sig['model_count']}"],
        [
            _rem(
                "rem-model",
                "Model inventory and access controls",
                "Track models, restrict extraction, monitor usage.",
                [],
            )
        ],
        w,
...
def score_hallucination(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    return _factor(
        ...
        "Hallucination Risk",
        base * w,
        "Ungrounded generative outputs can be incorrect and over-trusted.",
        ["grounding_signals_checked"],
        [
            _rem(
                "rem-hall",
                "Add grounding and citation checks",
                "Require retrieval grounding and human review for high-impact answers.",
                [],
            )
        ],
        w,
...
def score_supply_chain(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.25
        + (0.1 * min(3, len(sig["frameworks"])))
        + (
            0.15
            if "plugin" in sig["haystack"] or "marketplace" in sig["haystack"]
            else 0
        )
    )
    ...
    return _factor(
        ...
        "Supply Chain Risk",
        base * w,
        "Third-party models, plugins, and frameworks expand supply-chain attack surface.",
        [f"frameworks={sig['frameworks']}"],
        [
            _rem(
                "rem-sc",
                "Vendor and dependency controls",
                "Pin versions, scan packages, vet model providers.",
                [],
            )
        ],
        w,
...
def score_shadow_ai(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    return _factor(
        ...
        "Shadow AI Risk",
        base * w,
        "Unsanctioned AI usage bypasses enterprise controls.",
        [f"shadow_ai={sig['shadow_ai']}"],
        [
            _rem(
                "rem-shadow",
                "Discover and govern Shadow AI",
                "CASB/DLP discovery, approved AI catalog, policy enforcement.",
                [],
            )
        ],
        w,
...
def score_runtime(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.2
        + (0.25 if sig["public_runtime"] else 0)
        + (0.15 if "kubernetes" in sig["haystack"] else 0.05)
    )
    ...
    return _factor(
        ...
        "Runtime Risk",
        base * w,
        "Runtime exposure and missing admission controls elevate operational compromise risk.",
        [f"public_runtime={sig['public_runtime']}"],
        [
            _rem(
                "rem-rt",
                "OPA/runtime restrictions",
                "Egress allowlists, deny shell, budgets.",
                ["gr-opa-runtime", "gr-rate-limit"],
            )
        ],
        w,
    )
...
def score_autonomy(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    return _factor(
        ...
        "Autonomy Risk",
        base * w,
        "Autonomous multi-agent systems can take high-impact actions without oversight.",
        [f"autonomy={sig['has_autonomy']}"],
        [
            _rem(
                "rem-auto",
                "Human-in-the-loop for high-impact actions",
                "Cap iterations; require approvals.",
                ["gr-tool-allowlist"],
                "P0",
            )
        ],
...
def score_business(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    ...
    return _factor(
        ...
        "Business Risk",
        base * w,
        "Business criticality amplifies impact of AI failures or breaches.",
        [f"critical={sig['business_critical']}"],
        [
            _rem(
                "rem-biz",
                "Business impact assessments",
                "Tier AI systems; apply stricter controls to critical tiers.",
                [],
            )
        ],
        w,
...
def score_operational(sig: dict[str, Any], weights: dict[str, float]) -> RiskFactor:
    base = (
        0.25
        + (0.2 if sig["model_count"] > 2 else 0.05)
        + (0.15 if sig["has_autonomy"] else 0)
    )
    ...
    return _factor(
        ...
        "Operational Risk",
        base * w,
        "Operational complexity from models/agents increases failure and cost risk.",
        [f"models={sig['model_count']}"],
        [
            _rem(
                "rem-ops",
                "SLOs, budgets, and observability",
                "Rate limits, tracing, cost controls.",
                ["gr-rate-limit"],
            )
        ],
        w,
