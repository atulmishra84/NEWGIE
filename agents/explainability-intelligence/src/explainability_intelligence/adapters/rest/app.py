...
from explainability_intelligence.adapters.rest.errors import register_exception_handlers
from explainability_intelligence.adapters.rest.middleware import ObservabilityMiddleware
from explainability_intelligence.adapters.rest.routes_explain import alias, router
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="GIE Explainability Intelligence Agent",
        version=AGENT_VERSION,
        description="Explain every decision made by every agent (gie.explainability.v1)",
        lifespan=lifespan,
    )
