...
ROLE_PERMS = {
    ...
    Role.ANALYST: frozenset(
        {RecommendationPermission.REC_READ, RecommendationPermission.REC_GENERATE}
    ),
    Role.OPERATOR: frozenset(
        {
            RecommendationPermission.REC_READ,
            RecommendationPermission.REC_GENERATE,
            RecommendationPermission.REC_APPROVE,
        }
    ),
...
def require_recommendation_permission(
    roles, permission: RecommendationPermission
) -> None:
    granted = set()
    for name in roles:
        try:
