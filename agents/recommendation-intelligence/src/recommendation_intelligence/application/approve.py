...
from recommendation_intelligence.application.errors import (
    NotFoundError,
    RecommendationError,
)
...
class ApproveRecommendationsHandler:
    ...
    async def handle(
        self, request: RecommendationApproveRequest, *, actor: str, correlation_id: str
    ) -> RecommendationReport:
        prior = await self._reports.latest_for_agent(
            request.tenant_id, request.agent_id
        )
        if not prior:
            raise NotFoundError(
                f"No recommendations for agent {request.agent_id}; generate first"
            )
        if not request.approve_all and not request.recommendation_ids:
            raise RecommendationError(
                "invalid_request", "Provide recommendation_ids or approve_all=true"
            )
        ...
        approved_ids = [
            i.recommendation_id
            for i in report.recommendations
            if i.status.value == "approved"
        ]
