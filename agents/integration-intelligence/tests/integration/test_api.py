...
async def test_catalog_connect_sync_webhook_auth_audit(sample_connect):
    settings = Settings(gie_env="test", require_auth=False, webhook_hmac_secret="whsec", retry_max_attempts=2, retry_base_delay_ms=1)
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        sync = await client.post(
            ...
            json={"tenant_id": "acme", "payload": {"kind": "policy_bundle", "count": 2}},
            headers={"x-tenant-id": "acme"},
        )
        assert sync.status_code == 200
        assert sync.json()["data"]["status"] == "success"

        payload = {"tenant_id": "acme", "event_type": "push", "payload": {"ref": "main"}}
        raw = json.dumps(payload).encode()
        sig = "sha256=" + hmac.new(b"whsec", raw, hashlib.sha256).hexdigest()
        wh = await client.post(
            "/webhooks/github",
            content=raw,
            headers={"x-tenant-id": "acme", "content-type": "application/json", "x-gie-signature": sig},
        )
        ...
        tok = await client.post(
            "/auth/token",
            json={"tenant_id": "acme", "auth_method": "jwt", "subject": "svc", "scopes": ["gie.read"]},
            headers={"x-tenant-id": "acme"},
        )
        ...
        health = await client.get("/integrations/health/report", headers={"x-tenant-id": "acme"})
