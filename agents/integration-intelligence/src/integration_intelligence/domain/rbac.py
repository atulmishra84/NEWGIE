...
_ROLE_PERMS: dict[str, set[IntegrationPermission]] = {
    "viewer": {IntegrationPermission.INTEGRATION_READ, IntegrationPermission.INTEGRATION_AUDIT},
    "developer": {
...
def require_integration_permission(roles: set[str] | frozenset[str], permission: IntegrationPermission) -> None:
    allowed: set[IntegrationPermission] = set()
    for r in roles:
        allowed |= _ROLE_PERMS.get(r, set())
    if permission not in allowed and IntegrationPermission.INTEGRATION_ADMIN not in allowed:
