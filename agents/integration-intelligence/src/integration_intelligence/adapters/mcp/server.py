...
TOOLS = [
    {
        "name": "list_platforms",
        "description": "List supported GIE integration platforms",
        "inputSchema": {"type": "object", "properties": {}},
    },
    {
        "name": "connect_platform",
        "description": "Connect an enterprise platform",
        "inputSchema": {
            "type": "object",
    ...
    {
        "name": "issue_auth_token",
        "description": "Issue OAuth/API key/JWT/mTLS token",
        "inputSchema": {
            "type": "object",
            "properties": {"request": {"type": "object"}},
            "required": ["request"],
    ...
    {
        "name": "ingest_webhook",
        "description": "Ingest a webhook event",
        "inputSchema": {
            "type": "object",
            "properties": {
...
async def call_tool(name: str, arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    if name == "list_platforms":
        ...
        return {
            "count": len(plats),
            "platforms": [p.model_dump(mode="json") for p in plats],
        }
    ...
    if name == "ingest_webhook":
        ...
        raw = b"{}"
        event = await c.webhook.handle(
            pid,
            body,
            raw_body=raw,
            actor="mcp",
            correlation_id=uuid4().hex,
            tenant_fallback=arguments.get("tenant_id") or "default",
        )
