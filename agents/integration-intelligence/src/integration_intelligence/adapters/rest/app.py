...
from integration_intelligence.adapters.rest.errors import register_exception_handlers
from integration_intelligence.adapters.rest.middleware import ObservabilityMiddleware
from integration_intelligence.adapters.rest.routes_integration import alias, router
from integration_intelligence.infrastructure.bootstrap import build_container
from integration_intelligence.settings import get_settings
...
def create_app() -> FastAPI:
    app = FastAPI(
        title="GIE Integration Intelligence Agent",
        version=AGENT_VERSION,
        description="Enterprise platform integrations for Guardrails Intelligence Engine (gie.integration.v1)",
        lifespan=lifespan,
    )
    app.add_middleware(ObservabilityMiddleware)
    register_exception_handlers(app)
    # static health-report path must be registered; router order: include after defining static before param routes
