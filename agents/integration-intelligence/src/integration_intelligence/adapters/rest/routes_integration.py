...
async def list_integrations(
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_READ)
    ),
):
...
async def connect(
    body: ConnectRequest,
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_CONNECT)
    ),
    ...
    conn = await container.connect.handle(
...
async def list_connections(
    ...
    started = time.perf_counter()
    items = await container.connections.list(
...
async def sync_connection(
    ...
    body: SyncRequest,
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_SYNC)
    ),
    container: Container = Depends(container_dep),
):
    started = time.perf_counter()
    ...
    if not body.tenant_id:
        body.tenant_id = principal.tenant_id
    result = await container.sync.handle(
        body, actor=principal.subject_id, correlation_id=uuid4().hex
    )
...
async def webhook_deliveries(
    ...
    started = time.perf_counter()
    items = await container.webhooks.list(
        principal.tenant_id, limit=limit, offset=offset
    )
    return ObservabilityEnvelope(
        data={"items": items, "count": len(items)}, meta=_meta(started)
...
async def webhook_ingress(
    ...
    request: Request,
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_WEBHOOK)
    ),
    container: Container = Depends(container_dep),
):
    started = time.perf_counter()
...
async def auth_token(
    ...
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_AUTH)
    ),
    container: Container = Depends(container_dep),
):
    started = time.perf_counter()
...
async def audit_logs(
    ...
    offset: int = Query(0, ge=0),
    principal: AuthPrincipal = Depends(
        require_perm(IntegrationPermission.INTEGRATION_AUDIT)
    ),
    container: Container = Depends(container_dep),
):
    started = time.perf_counter()
