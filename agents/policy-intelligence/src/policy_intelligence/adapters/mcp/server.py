...
from gie_contracts.policy import PolicyGenerateRequest, PolicyInputBundle
from policy_intelligence.application.di import get_container

TOOLS = [
    {
        "name": "policy_generate",
        "description": "Generate deployment-ready guardrail policies from context/risk/compliance/knowledge/identity/business inputs",
        "inputSchema": {
            "type": "object",
            "properties": {
                "bundle": {"type": "object"},
    ...
    {
        "name": "policy_explain",
        "description": "Explain why guardrails apply for a decision",
        "inputSchema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "policy_get_artifact",
        "description": "Fetch a generated policy artifact by filename",
        "inputSchema": {
            "type": "object",
            "properties": {
                "decision_id": {"type": "string"},
...
async def call_tool(
    ...
    if name == "policy_generate":
        ...
        req = PolicyGenerateRequest(
            bundle=bundle, dry_run=bool(arguments.get("dry_run", False))
        )
    ...
    if name == "policy_explain":
        ...
        return await c.explain.handle(
            UUID(arguments["decision_id"]), arguments.get("guardrail_id")
        )
