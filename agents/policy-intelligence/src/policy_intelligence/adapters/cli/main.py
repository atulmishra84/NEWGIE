...
@app.command()
def generate(
    input_json: Path, out_dir: Path = Path("./policy-out"), memory: bool = True
):
...
async def _generate(input_json: Path, out_dir: Path, memory: bool):
    ...
    bundle = PolicyInputBundle.model_validate(
    ...
    decision = await container.generate.handle(
        PolicyGenerateRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex
    )
    ...
    rprint(
        f"[green]Wrote {len(decision.artifacts)} artifacts + decision.json to {out_dir}[/green]"
    )
