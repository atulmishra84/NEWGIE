...
def render(recs: list[GuardrailRecommendation], bundle: PolicyInputBundle) -> dict[str, Any]:
    return {
        ...
        "autogen": {
            "code_execution_config": {"use_docker": True, "timeout": 60} if any(r.guardrail_id=="gr-tool-allowlist" for r in recs) else False,
            "human_input_mode": "ALWAYS" if any(r.priority.value=="P0" for r in recs) else "TERMINATE",
