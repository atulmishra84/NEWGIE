...
class GeneratePolicyHandler:
    ...
    async def handle(self, request: PolicyGenerateRequest, *, actor: str, correlation_id: str) -> PolicyDecision:
        ...
        if not request.dry_run:
            ...
            await self._cache.set_json(cache_key, decision.model_dump(mode="json"), self._settings.cache_ttl_seconds)
            ...
            await self._events.publish(self._settings.kafka_topic_events, evt.model_dump(mode="json"), key=str(decision.decision_id))
        logger.info(
            "policy_generated",
