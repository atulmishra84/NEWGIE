...
async def generate(
    ...
    principal: AuthPrincipal = Depends(
        require_perm(PolicyGenPermission.POLICY_GENERATE)
    ),
    ...
    package = await container.generate.handle(
        body, actor=principal.subject_id, correlation_id=uuid4().hex
    )
    return ObservabilityEnvelope(
        data=package,
        meta=_meta(started, package.confidence.score, package.reasoning_path),
    )
...
async def validate(
    ...
    principal: AuthPrincipal = Depends(
        require_perm(PolicyGenPermission.POLICY_VALIDATE)
    ),
    container: Container = Depends(container_dep),
):
    started = time.perf_counter()
    result = await container.validate.handle(
        body, actor=principal.subject_id, correlation_id=uuid4().hex
    )
