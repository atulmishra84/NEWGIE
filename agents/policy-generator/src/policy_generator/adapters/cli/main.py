...
from gie_contracts.policy_generator import PolicyPackageGenerateRequest, PolicyGeneratorInputBundle
...
async def _generate(input_json: Path, out: Path):
    ...
    bundle = PolicyGeneratorInputBundle.model_validate(raw if "agent_id" in raw else raw.get("bundle", raw))
    c = await build_container(memory=True)
    pkg = await c.generate.handle(PolicyPackageGenerateRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex)
