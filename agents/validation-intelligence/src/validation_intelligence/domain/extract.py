...
def collect_documents(bundle: ValidationInputBundle) -> list[dict[str, Any]]:
    ...
    for p in pkg.get("policies") or bundle.policies or []:
        if isinstance(p, dict):
            docs.append(
                {
                    "filename": p.get("filename") or p.get("metadata", {}).get("name") or "policy.json",
    ...
    if isinstance(named, dict):
        for fname, content in named.items():
            docs.append({"filename": fname, "format": _guess_format(fname), "content": content or "", "body": {}})
    if bundle.content:
        docs.append(
            {
    ...
    if not docs:
        docs.append({"filename": "empty", "format": "json", "content": "{}", "body": {}})
    # parse bodies
    for d in docs:
        if d["body"]:
            continue
        ...
        try:
            if fmt in {"json", "openai_guardrails", "azure_ai_foundry", "langgraph", "crewai", "autogen", "semantic_kernel", "api_gateway", "prompt", "identity", "runtime", "dlp"} or (
                content.lstrip().startswith("{")
            ):
                d["body"] = json.loads(content) if content.strip() else {}
            elif fmt in {"yaml", "kubernetes", "admission_controller"} or "apiVersion:" in content[:200]:
...
def extract_rules(docs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for d in docs:
        ...
        if isinstance(body.get("spec"), dict) and isinstance(body["spec"].get("rules"), dict):
            for k, v in body["spec"]["rules"].items():
                candidates.append({"id": k, "value": v, "source": d["filename"]})
        for step in body.get("steps") or []:
            if isinstance(step, dict):
                candidates.append({"id": step.get("type") or str(step), "value": step, "source": d["filename"]})
        controls = body.get("controls") or (body.get("spec") or {}).get("controls") or []
        for c in controls:
            candidates.append({"id": str(c), "value": True, "source": d["filename"]})
        if body.get("rego"):
            candidates.append({"id": "rego.allow", "value": "allow" in str(body["rego"]), "source": d["filename"]})
