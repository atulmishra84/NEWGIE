...
from validation_intelligence.application.errors import NotFoundError, ValidationError
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def ve(request: Request, exc: ValidationError):
...
def _err(request, code, message, retryable=False):
    ...
    return {"code": code, "message": message, "retryable": retryable, "meta": {"trace_id": getattr(obs, "trace_id", uuid4().hex), "request_id": getattr(obs, "request_id", uuid4().hex), "correlation_id": getattr(obs, "correlation_id", uuid4().hex), "execution_ms": (time.perf_counter() - started) * 1000, "agent_version": AGENT_VERSION, "agent_name": "validation-intelligence"}}
