...
async def _validate(input_json: Path, out: Path):
    ...
    bundle = ValidationInputBundle.model_validate(raw if "tenant_id" in raw else raw.get("bundle", raw))
    c = await build_container(memory=True)
    report = await c.validate.handle(ValidateRequest(bundle=bundle), actor="cli", correlation_id=uuid4().hex)
