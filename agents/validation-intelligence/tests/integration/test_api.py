...
async def test_validate_simulate_get_report(sample_bundle):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        r = await client.post(
            "/validate",
            json={"bundle": sample_bundle.model_dump(mode="json"), "persist": True, "run_simulation": True},
        ...
        report = await client.get("/validation/report", params={"agent_id": "agent-checkout-bot"}, headers={"x-tenant-id": "acme"})
        assert report.status_code == 200
        assert "approval_status" in report.json()["data"]

        sim = await client.post(
            "/v1/simulate",
            json={"tenant_id": "acme", "agent_id": "agent-checkout-bot", "policy_package": sample_bundle.policy_package},
            headers={"x-tenant-id": "acme"},
        )
