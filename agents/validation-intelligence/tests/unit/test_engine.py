...
def test_validate_pass_path(sample_bundle):
    ...
    assert report.approval_status in {ApprovalStatus.APPROVED, ApprovalStatus.APPROVED_WITH_WARNINGS}
    cats = {c.category.value for c in report.checks}
    for required in [
        "syntax", "schema", "compliance", "runtime_compatibility", "policy_conflicts",
        "duplicate_rules", "performance", "security", "framework_compatibility", "model_compatibility", "simulation",
    ]:
